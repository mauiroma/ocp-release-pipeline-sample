...
"""
                        script {
                            withCredentials([string(credentialsId: 'jenkins-token-semplificazione', variable: 'SECRET_OCP_SERVICE_TOKEN')]) {
                                def buildconfigUpdateResult =
                                        sh(script: "oc patch bc eap71-nautilus  -p '{\"spec\":{\"output\":{\"to\":{\"kind\":\"ImageStreamTag\",\"name\":\"eap71-nautilus:${BUILD_TAG}\"}}}}' --namespace=semplificazione -o json --token=$SECRET_OCP_SERVICE_TOKEN $target_cluster_flags |oc replace eap71-nautilus --namespace=semplificazione $target_cluster_flags --token=$SECRET_OCP_SERVICE_TOKEN -f -",
                                                returnStdout: true)
        ...
        stage('deploy') {
            steps {
                script {
                    withCredentials([string(credentialsId: 'jenkins-token-semplificazione', variable: 'SECRET_OCP_SERVICE_TOKEN')]) {
                        def patchIamgeStream = sh(script: "oc set image dc/eap71-nautilus eap71-nautilus=docker-registry.default.svc:5000/semplificazione/eap71-nautilus:${BUILD_TAG} --namespace=semplificazione --token=$SECRET_OCP_SERVICE_TOKEN $target_cluster_flags",
                                returnStdout: true)
                        ...
                        if (!patchIamgeStream?.trim()) {
                            ...
                            if (!currentImageStreamVersion.equalsIgnoreCase("docker-registry.default.svc:5000/semplificazione/eap71-nautilus:${BUILD_TAG}")) {
                        ...
                        //oc get dc  eap71-nautilus -o jsonpath='{.spec.template.spec.containers[0].image}'
                    }
