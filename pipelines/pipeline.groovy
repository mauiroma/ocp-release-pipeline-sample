                        ...
                        script {
                            withCredentials([string(credentialsId: "${OCP_SERVICE_TOKEN}", variable: 'OCP_SERVICE_TOKEN')]) {
                                def patchIamgeStream = 
                                    sh(
                                        script: "oc set image dc/${OCP_BUILD_NAME} ${OCP_BUILD_NAME}=docker-registry.default.svc:5000/${OCP_PRJ_NAMESPACE}/${OCP_BUILD_NAME}:${BUILD_TAG} --token=${OCP_SERVICE_TOKEN} $target_cluster_flags",
                                        returnStdout: true
                                    )
                                ...
                                if (!patchIamgeStream?.trim()) {
                                    ...
                                    if (!currentImageStreamVersion.equalsIgnoreCase("docker-registry.default.svc:5000/${OCP_PRJ_NAMESPACE}/${OCP_BUILD_NAME}:${BUILD_TAG}")) {
